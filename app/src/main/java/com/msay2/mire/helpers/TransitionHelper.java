...
public class TransitionHelper
{
    ...
    public static void startExitAnim(View root)
	{
		...
		exitAnimate((DepthFAB)root.findViewById(R.id.fab_container), MOVE_Y_STEP * 2f, 20f, 45, 210, true);
...
//        decorView.setSystemUiVisibility(uiOptions);
	...
	static ValueAnimator exitAnimate(final DepthFAB target, final float moveY, final float customElevation, long delay, int subtractDelay, boolean continueOffscreen)
	{
        target.setPivotY(getDistanceToCenter(target));
        target.setPivotX(getDistanceToCenterX(target));
        target.setCameraDistance(10000 * target.getResources().getDisplayMetrics().density);
    ...
    public static void animateToMenuState(View root, AnimatorListenerAdapter onMenuAnimFinished) 
	{
		...
		exitAnimate((DepthFAB)root.findViewById(R.id.fab_container), MOVE_Y_STEP * 2f, 20f, 45, 210, false);
...
//                        | View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN);
	...
	static ObjectAnimator revertFromMenu(final DepthFAB target, final float customElevation, int subtractDelay, float targetElevation) 
	{
        target.setPivotY(getDistanceToCenter(target));
        target.setPivotX(getDistanceToCenterX(target));
        target.setCameraDistance(10000 * target.getResources().getDisplayMetrics().density);
